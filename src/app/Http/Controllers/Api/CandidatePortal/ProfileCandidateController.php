...
class ProfileCandidateController extends Controller
{
    ...
    public function index(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            if ($id == 3) {
                $ids = Matching::distinct()->get()->pluck('profile_id');
    ...
    public function getChartsStats(Request $request)
    {
        ...
        return $this->compareStats($totalStats, $currentStats, $previousStats, $currentYear, $lastYear);
    }
    public function defaultGetChartsStats()
    {
        ...
        // Compare stats and calculate percentage changes
        return $this->compareStats($totalStats, $currentStats, $lastYearStats, $currentYear, $lastYear);
    }
    ...
    private function compareStats($totalStats, $currentStats, $previousStats, $currentYear, $lastYear)
    {


        $currentMatchingStats = $this->getMatchingStats($currentYear);
        $lastmatchingYearStats = $this->getMatchingStats($lastYear);
        //  dd($lastmatchingYearStats);

        $totalPertinent = Matching::distinct()->count('profile_id');

        // dd($this->calculateChange($currentMatchingStats->total_matched_profiles, $lastmatchingYearStats->total_matched_profiles));
        return [
            'cvtheque' => [
