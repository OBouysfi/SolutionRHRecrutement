...
const now = new Date();
const endDate = now.toLocaleDateString('fr-FR'); // Format: JJ/MM/AAAA
...
$('#titlecalendar, #titlecalendar1').daterangepicker({
    ...
    if ($('.btn-refresh').length === 0) {
        ...
        var refreshButton = $(`
            <button class="btn btn-outline-theme btn-refresh" id="refresh-date-fillter" style="margin: 0 5px; padding:2px 5px 0px;">
                ${(t.clear || "Effacer")} <i class="bi bi-arrow-clockwise"></i>
            </button>
        `);

        picker.container.find('.drp-buttons').find('button.applyBtn').before(refreshButton);
    }
});
