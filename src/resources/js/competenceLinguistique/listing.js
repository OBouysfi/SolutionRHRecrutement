...
$(document).ready(function () {
    ...
    table = $('#linguistiqueTable').DataTable({
        processing: false,
        serverSide: true,
        lengthChange: false,
        searching: false,
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                d.search = $('#customSearchBox-linguistique').val();
            },
        ...
        columns: [
            { data: "linguistique", name: "linguistique"  ,orderable: false, },
            { data: "category", name: "category"  ,orderable: false, },
            { data: 'action', name: 'action'  ,orderable: false,}
        ],
        drawCallback: function (settings) {
            updateCustomPagination(settings);
             $('#linguistiqueTable thead th').each(function () {
                    const $th = $(this);
    ...
    function updateCustomPagination(settings) {
        ...
        const paginationWrapper = $('#validated-linguistiquesTable-pagination .pagination');
        ...
        $('#validated-linguistiquesTable-pagination .label').text(`${currentPage} sur ${totalPages}`);
    ...
    function addPaginationEventListeners() {
        $('#validated-linguistiquesTable-pagination .footable-page-nav').off('click').on('click', function (e) {
            e.preventDefault();
        ...
        $('#validated-linguistiquesTable-pagination .footable-page').off('click').on('click', function (e) {
            e.preventDefault();
    ...
    $('#customSearchBox-linguistique').on('input', function () {
            table.ajax.reload(); 
            });
...
$(document).ready(function () {
    $("body").on("click", ".edit-btn", function () {
        ...
        $.get("competences-linguistiques/" + langId + "/edit", function (data) {
            $("#editmodal").modal("show");
            $("#langId").val(data.id);
            $("#name_lang").val(data.label);
            $("#edit_skill_type_id").val(data.skill_type_id);
            $("#edit_skill_type_id").trigger("chosen:updated");
    ...
    $("#save_edit").click(function (e) {
        ...
        var updateUrl = langUpdateUrl.replace('__ID__', langId); 
    
        var formData = {
            label: $("#name_lang").val(),
            skill_type_id: $("#edit_skill_type_id").val(),
        };
...
window.deletelang = function (id) {
    ...
    var urlToDelete = langDestroy.replace('__ID__', id);
 ...
 * Js For Show
 */
window.viewlang = function (langId) {
    var viewUrl = langView.replace("__ID__", langId);
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (response) {
            $('#langeViewModal').modal('show');
            if (response && response.label && response.show_skill_type_id) {
                $('#label').text(response.label);  
                $('#show_skill_type_id').text(response.show_skill_type_id);  
            } else {
                $('#label').text('Données non disponibles');
                $('#show_skill_type_id').text('Données non disponibles');
            }
        },
        error: function (xhr, status, error) {
            console.error('Erreur AJAX:', status, error);
            $('#label').text(window.translations.failed_to_load_data);
            $('#show_skill_type_id').text(window.translations.failed_to_load_data);
        }
    });
};
