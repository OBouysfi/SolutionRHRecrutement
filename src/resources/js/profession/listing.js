...
$(document).ready(function () {
    ...
    var table = $('#professionsTable').DataTable({
        processing: false,
        serverSide: true,
        lengthChange: false,
        searching: false,
        ...
        ajax: {
            url: professionsData,
            data: function (d) {
                d.search = $('#customSearchBox-profession').val();
        ...
        columns: [
            { data: "Métier/poste", name: "label" ,orderable: false, },
            { data: 'action', name: 'action' ,orderable: false, },

        ],
        drawCallback: function (settings) {
            updateCustomPagination(settings);
             $('#professionsTable thead th').each(function () {
                    const $th = $(this);
    ...
    function updateCustomPagination(settings) {
        ...
        const currentPage = Math.floor(settings._iDisplayStart / pageLength) + 1; // 1-based page index
    ...
    $('#customSearchBox-profession').on('input', function () {
    table.ajax.reload(); 
    });
    ...
    window.deleteProfession = function (profession) {
        console.log("Tentative de suppression de l'ID :", profession);

        var urlToDelete = professionsDestroy.replace('__ID__', profession);
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        Swal.fire({
                            ...
                            text: window.translations.tech_skill_error,
                            icon: "success",
                            confirmButtonText: window.translations.confirm
                        });
                        $('#professionsTable').DataTable().ajax.reload(null, false);
                    },
                    error: function (xhr) {
    ...
    /**
    * Js For Show Profession
    */
    window.viewDetails = function (profession) {
        var url = professionsView.replace('__ID__', profession);
        $.ajax({
            url: url,
            type: 'GET',
            dataType: 'json',
            success: function (response) {
                $('#professionViewModal').modal('show');
                $('#professionDetails').empty();
                if (typeof response === 'object' && response !== null) {
                    $.each(response, function (label, value) {
                        $('#professionDetails').append(
                            `<p><strong>${label}:</strong> ${value}</p>`
                        );
                    });
                } else {
                    $('#professionDetails').text('Invalid response format');
                }
            },
            error: function (xhr, status, error) {
                console.error(window.translations.mis_ajour, error);
                $('#professionDetails').text(window.translations.tech_failed);
            }
        });
    };
