...
$(document).ready(function () {
    ...
    var table = $('#trackingApplicationTable').DataTable({
        ...
        columns: [
            { data: 'select_check', orderable: false, searchable: false },
            { data: 'picture', orderable: false, searchable: false },
            { data: 'matricule', name: 'profiles.matricule' },
            { data: 'first_name', name: 'profiles.first_name' },
            { data: 'last_name', name: 'profiles.last_name' },
            { data: 'diploma', name: 'diploma' },
            { data: 'option', name: 'option' },
            { data: 'experience', name: 'experience' },
            { data: 'current_position', name: 'current_position' },
            { data: 'desired_position', name: 'desired_position' },
            { data: 'score', orderable: false, searchable: false },
            { data: 'status', name: 'tracking_applications.status' },
...
$('#customLength').on('change', function () {
    ...
    function updateCustomPagination(settings) {
        ...
        // Ajouter les boutons "Suivant" et "Dernier"
        paginationWrapper.append(`
            <li class="footable-page-nav ${currentPage === totalPages ? 'disabled' : ''}" data-page="next">
                <a class="footable-page-link" href="#">›</a>
            </li>
        ...
        // Ajouter les événements pour les boutons de pagination
        addPaginationEventListeners();
    }
    ...
    function addPaginationEventListeners() {
        $('#footable-pagination .footable-page-nav').on('click', function (e) {
            e.preventDefault();
            const action = $(this).data('page');
            const table = $('#vivierTalent-listing-table').DataTable();
    ...
    // $('#createProfileEvent').on('click', function () {
    window.createProfileEvent = function () {
        ...
        const translations = {
            fr: {
                selectAtLeastOneProfile: 'Veuillez sélectionner au moins un profil.',
                confirmButton: 'Ok',
            },
            en: {
                selectAtLeastOneProfile: 'Please select at least one profile.',
                confirmButton: 'Ok',
            },
            es: {
                selectAtLeastOneProfile: 'Por favor, seleccione al menos un perfil.',
                confirmButton: 'Ok',
            },
            pt: {
                selectAtLeastOneProfile: 'Por favor, selecione pelo menos um perfil.',
                confirmButton: 'Ok',
            },
            ar: {
                selectAtLeastOneProfile: 'يرجى اختيار ملف شخصي واحد على الأقل.',
                confirmButton: 'موافق',
            },
            zh: {
                selectAtLeastOneProfile: '请至少选择一个配置文件。',
                confirmButton: '好的',
            },
        };
        ...
        if(selectedIds.length === 0){
                Swal.fire({
                    icon: 'warning',
                    title: t.selectAtLeastOneProfile,
...
window.saveEvent = function (type) {
    ...
    // Translation object for save event confirmation and messages
    const translations = {
        fr: {
            successTitle: 'Succès',
            successMessage: 'Le formulaire a été sauvegardé avec succès.',
        ...
        es: {
            successTitle: 'Éxito',
            successMessage: 'El formulario ha sido guardado exitosamente.',
        ...
        pt: {
            ...
            successMessage: 'O formulário foi salvo com sucesso.',
        ...
        zh: {
            successTitle: '成功',
            successMessage: '表单已成功保存。',
            validationErrorTitle: '验证错误',
            errorTitle: '错误',
            errorMessage: '发生了一个意想不到的错误。',
        },
    };
    const locale = document.documentElement.lang || "fr";
    const t = translations[locale] || translations.fr;
        ...
        error: function (xhr) {
            ...
            if (xhr.status === 422) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: t.errorTitle,
                    text: errorResponse.error || t.errorMessage,
                });
            }
        },
    });
            ...
            $('.to-change-meet .message-div').html(`
                <div class="alert alert-warning">
                    <div class="row">
                        ...
                        <div class="col-10">
                            <p>Connectez votre agenda Google pour générer un lien de réunion Google Meet.&nbsp;&nbsp;
                            <span style="color: blue; cursor: pointer;" class="connect-google">Se connecter</span></p>
                        </div>
                    </div>
                </div>
            `);
            ...
            $('.to-change-meet .message-div').html(`
                <div class="alert alert-warning">
                    <div class="row">
                        <div class="col-auto"><i class="bi bi-info-circle"></i></div>
                        <div class="col-10">
                            <p>Connectez votre agenda Office 365 pour générer un lien de réunion Microsoft Teams.&nbsp;&nbsp;
                            <span style="color: blue; cursor: pointer;" class="connect-teams">Se connecter</span></p>
                        </div>
                    </div>
                </div>
            `);
            ...
            $('.to-change-meet .message-div').html(`
                <div class="alert alert-warning">
                    <div class="row">
                        <div class="col-auto"><i class="bi bi-info-circle"></i></div>
                        <div class="col-10">
                            <p>Connectez votre agenda Zoom pour générer un lien de réunion Zoom.&nbsp;&nbsp;
                            <span style="color: blue; cursor: pointer;" class="connect-zoom">Se connecter</span></p>
                        </div>
                    </div>
                </div>
            `);
...
$(document).on('change', '.steps-chose', function () {
    ...
    const translations = {
        fr: {
            ...
            updateSuccessText: (status) => `Le statut a été mis à jour en ${status}`,
        ...
        en: {
            ...
            updateSuccessText: (status) => `The status has been updated to ${status}`,
        ...
        es: {
            ...
            updateSuccessText: (status) => `El estado se ha actualizado a ${status}`,
            updateErrorTitle: 'Error',
        ...
        pt: {
            ...
            updateSuccessText: (status) => `O status foi atualizado para ${status}`,
        ...
        ar: {
            ...
            updateSuccessText: (status) => `تم تحديث الحالة إلى ${status}`,
        ...
        zh: {
            ...
            updateSuccessText: (status) => `状态已更新为 ${status}`,
            updateErrorTitle: '错误',
            cancelInfoTitle: '已取消',
