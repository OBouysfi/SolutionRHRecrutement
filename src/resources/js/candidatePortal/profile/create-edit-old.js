...
$(window).on("load", function () {
    ...
    // const civillitySection = document.querySelector('.civillity-section');
    ...
    //                 civillitySection?.style.display = 'flex';
    //                 nameSection?.style.display = 'none';
    //             } else {
    //                 civillitySection?.style.display = 'none';
    ...
    `,
        ...
        $.ajax({
            url: "/api/candidate-portal/profiles/parse-cv",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                ...
                if (response.profile_id) {
                    window.location.href = "/candidate-portal/profiles/edit";
                } else {
                    Swal.fire({
                        confirmButtonText: window.translations.confirm,
                        icon: 'error',
    ...
    $("#technical-category").on("change", function () {
        ...
        if (selectedTypeId === "Tous") {
            ...
            $(
                '#technical-skill option[data-skill-type="' +
                selectedTypeId +
                '"]'
            ).show();
        }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.avatar_url) {
                        // Update the avatar image `src`
                        $(".avatar-figure img.profile-avatar").attr(
                            "src",
                            response.avatar_url + "?" + new Date().getTime()
                        );
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.cover_url) {
                        ...
                        $(".cover-figure img.profile-cover").attr(
                            "src",
                            response.cover_url + "?" + new Date().getTime()
                        );
    ...
    $(document).on("change", ".custom-file-input", function () {
        ...
        if (file) {
            ...
            figure.css(
                "background-image",
                'url("/assets/img/icon/empty-logo.png")'
            );
        }
    });
...
$(document).on("click", "#saveSkills", function (e) {
    ...
    $.ajax({
        url: "/api/candidate-portal/profiles/store/skills",
        method: "POST",
        data: {
...
$(document).on("click", "#updateSkills", function (e) {
    ...
    // AJAX request to save data
    $.ajax({
        url: "/api/candidate-portal/profiles/update/skills",
        method: "POST",
        data: {
            _token: $('meta[name="csrf-token"]').attr("content"),
            technical_skills: technicalSkills,
...
$('select[name="country_id"]').on("change", function () {
    ...
    if (selectedCountryId === "Tous") {
        ...
        villeSelect
            .find('option[data-country="' + selectedCountryId + '"]')
            .show();
    }
...
//                             </select>
//                             <label class="translated_text">Catégorie</label>
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
//                     <div class="input-group input-group-lg">
//                         <div class="form-floating">
//                             <select class="form-select border-0 filled-personal-weight personal-weight" readonly>
//                                 <option value="${weight}" selected>${weight}</option>
//                             </select>
//                             <label class="translated_text">Échelle</label>
//                         </div>
...
document
    .querySelectorAll(".main-personal-skill-name")
    .forEach((skillSelect) => {
        skillSelect.addEventListener("change", function () {
            ...
            let levelSelect =
                skillContainer?.nextElementSibling?.querySelector(
                    ".personal-level"
                );
...
window.saveForm = function (formName, tabNumber) {
    ...
    $.ajax({
        url: `/api/candidate-portal/profiles/store/${formName}`, // Backend endpoint
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
            ...
            if (response && response[formName]) {
                if (formName === "formations") {
                    ...
                    document.querySelector(
                        `#profile-form-${formName} .coverimg`
...
document.querySelectorAll(".doc-file-input").forEach((input) => {
    input.addEventListener("change", function (event) {
        ...
        const exportButton = input
            .closest(".file-upload")
            .parentElement.parentElement.querySelector(".export-button");
...
async function getLangSkills(langId) {
    try {
        const response = await $.ajax({
            url: `/api/candidate-portal/profiles/lang-skills/${langId}`,
            type: "GET",
        });
                                    ...
                                    <div class="col-12 col-md-10">
                                        <div class="row">
                                            ...
                                            <div class="col-6 col-md-3 mb-2">
                                                <div class="custom-multiple-select rounded border poste-chosen">
                                                    <label class="translated_text">${window.translations.country}</label>
                                            ...
                                            <div class="col-6 col-md-3 mb-2">
                                                <div class="custom-multiple-select rounded border poste-chosen">
                                                    <label class="translated_text">${window.translations.city}</label>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6 col-md-3 mb-2  custom-mrg">
                                                <div class="custom-multiple-select rounded border poste-chosen">
                                                    <label class="translated_text">${window.translations.diploma} <span class="text-themeColor">*</span></label>
                                            ...
                                            <div class="col-12 col-md-6 mb-2 custom-mrg">
                                                <div class="custom-multiple-select rounded border poste-chosen">
                                                    <label class="translated_text">${window.translations.option}</label>
                                            ...
                                            <div class="col-6 col-md-3 mb-2">
                                                <div class="custom-multiple-select rounded border poste-chosen">
                                                    <label class="translated_text">${window.translations.mention}</label>
                    ...
                    <div class="row mt-4 justify-content-center">
                        ...
                        <div class="col-5">
                            <div class="row">
                                <div class="col-12 col-md-12 mb-2">
                                    <div class="custom-multiple-select rounded border poste-chosen">
                                        <label class="translated_text">${window.translations.evaluation_criteria} <span class="text-themeColor">*</span></label>
                                        <select class="form-select border-0" id="eval1" name="criteres_evaluation">
                                            <option selected value="0" id="null-value">${window.translations.evaluation_criteria}</option>
                        ...
                        <div class="col-5">
                            <div class="row">
                                <div class="col-12 col-md-12 mb-2">
                                    <div class="custom-multiple-select rounded border poste-chosen">
                                        <label class="translated_text">${window.translations.certification_level} <span class="text-themeColor">*</span></label>
                                        <div class="form-floating">
                                            <select class="form-select border-0 chosenoptgroup w-100" name="niveau_certification" id="country1">
                                                <option selected>${window.translations.certification_level}</option>
                                ...
                                <div class="col-12 col-md-6 col-lg-12 custom-mrg">
                                    <div class="form-group mb-3 position-relative is-valid check-valid">
                                        <div class="form-floating">
                                            ...
                                            <label class="translated_text">${window.translations.date_obtention}</label>
                                        </div>
                                    </div>
                                    <div class="invalid-feedback">${window.translations.please_enter_valid_input}</div>
                                </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-12 col-md-4 col-xl-3" style="margin-top: 15px;">
                    <div class="custom-multiple-select rounded border poste-chosen" style="border-bottom: 1px solid var(--adminux-theme) !important;border-radius: 8px !important;">
                        <label class="translated_text">${window.translations.post} <span class="text-themeColor">*</span></label>
                        <select class="chosenoptgroup w-100" name="profession_id" id="profession">
                            <option value="Tous" selected>${window.translations.choose_post}</option>
                            ${posts.map(post => `
                ...
                <div class="col-12 col-md-4 col-xl-2" style="padding-right: 0;margin-top: 15px;">
                    <div class="form-group mb-3 position-relative is-valid check-valid">
                        <div class="form-floating">
                ...
                <div class="col-12 col-md-6 col-lg-5">
                    <div class="form-group mb-3 position-relative is-valid check-valid">
                        <div class="form-floating">
                            ...
                            <label class="translated_text">${window.translations.technical_skills} <span class="text-themeColor">*</span></label>
                        </div>
                    </div>
                </div>
    ...
    <br>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-12 col-md-3 col-lg-3 mt-3">
                                                            <div class="custom-multiple-select rounded border poste-chosen"
                                                                ...
                                                                <label
                                                                 class="translated_text">Poste</label>
                                                                <select class="chosenoptgroup w-100"
                                                                    name="poste" id="profession">
                                                                    <option value="Tous" selected>Choisissez un poste
                                                                    </option>
                                                                    ${posts.map(post => `
            ...
            var cardItem = `       <div class="card mb-4 shadow-lg border-0 rounded-lg block-view bg-white" id="view-${formName}-${item.id}" style="background-color: white;">
                        <div class="card-body p-4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                ${photoHTML}
                                <div class="col-md-6">
                                    <h5 class="text-info mb-3">${item[titleKey] || "Non disponible"}</h5>
                                    ${fields
                    .map(field => `
                                            <p class="mb-2">
                                                <i class="${field.icon}"></i> <strong class="translated_text">${field.label} :</strong> ${resolveValue(item, field.key) || "Non disponible"}
                                            </p>
                                        `)
                    .join('')}
                                </div>
            ...
            const card = `
                <div>
                    ...
                    .custom-multiple-select .chosen-container .chosen-single {
        ...
        setTimeout(() => {
            ...
            if (formName === "languages") {
                document.getElementById('skills-form-container-' + item.id).innerHTML = generateSkillsForm(langSkills);
                // Initialize Select2 for the skills form
                setTimeout(() => {
                    initializeSelect2();
                    resetLanguageForm();
                }, 50);
...
window.saveAttentes = function () {
    ...
    try {
        ...
        formData.append(
            "profession_id",
            document.querySelector('select[name="desired_profession_id"]')
                .value || ""
        );
        formData.append(
            "contract_type",
            document.querySelector('select[name="contract_type"]').value || ""
        );
        formData.append(
            "activity_area_id",
            document.querySelector('select[name="activity_area_id"]').value ||
            ""
        );
        formData.append(
            "max_expected_salary",
            document.querySelector('input[name="max_expected_salary"]').value ||
            null
        );
        formData.append(
            "min_expected_salary",
            document.querySelector('input[name="min_expected_salary"]').value ||
            null
        );
        formData.append(
        ...
        formData.append(
            "company_size",
            document.querySelector('select[name="company_size"]').value || ""
        );
        formData.append(
            "has_driving_license",
            document.querySelector('select[name="has_driving_license"]')
                .value || ""
        );
        ...
        document
            .querySelectorAll('input[name^="mobility"]')
            .forEach((input) => {
                const [type, field] = input.name
                    .match(/\[(.*?)\]/g)
        ...
        document
            .querySelectorAll('input[name^="work_time"]')
            .forEach((input) => {
                const [type, field] = input.name
                    .match(/\[(.*?)\]/g)
                    .map((name) => name.replace(/\[|\]/g, ""));
                formData.append(
                    `work_time[${type}][${field}]`,
                    input.type === "checkbox"
                        ? input.checked
                        : input.value || ""
                );
        ...
        const availabilityTypeElement = document.querySelector(
            'input[name="availability[type]"]:checked'
        );
        if (availabilityTypeElement) {
            ...
            if (availabilityType === "notice") {
                const noticeDuration = document.querySelector(
                    'select[name="availability[notice_duration]"]'
                );
                if (noticeDuration) {
                    formData.append(
                        "availability[notice_duration]",
        ...
        $.ajax({
            url: `/api/candidate-portal/profiles/store/attentes`,
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                window.location.href = ProfileCandidateListingRoute;
            }, error: function (xhr) {
                if (xhr.status === 422) {
                    const errors = xhr.responseJSON.errors;
                    let errorMessages = '';
        ...
        const translations = {
            fr: {
                errorTitle: "Erreur",
                errorSave: "Impossible de sauvegarder les données. Veuillez vérifier le formulaire.",
            },
            en: {
                errorTitle: "Error",
                errorSave: "Unable to save data. Please check the form.",
            },
            ar: {
                errorTitle: "خطأ",
                errorSave: "تعذر حفظ البيانات. يرجى التحقق من النموذج.",
            },
            pt: {
                errorTitle: "Erro",
                errorSave: "Não foi possível salvar os dados. Por favor, verifique o formulário.",
            },
            zh: {
                errorTitle: "错误",
                errorSave: "无法保存数据。请检查表单。",
            },
            es: {
                errorTitle: "¡Error!",
                errorSave: "No se pudieron guardar los datos. Por favor, verifique el formulario.",
            }
        };
        const locale = document.documentElement.lang || "fr";
        const t = translations[locale] || translations.fr;
...
//     let formData = new FormData(form); // ✅ Captures files too!

//     // if (typeof CKEDITOR !== "undefined" && CKEDITOR.instances.cover_letter_ckeditor) {
//     //     CKEDITOR.instances.cover_letter_ckeditor.updateElement();
//     // }

//     $.ajax({
//         url: `/api/candidate-portal/profiles/update/${formName}`,
//         type: 'POST',
//         data: formData,
...
//                     recommandations = response.recommandations;
//                 }
//                 toggleFormationForm(formName, formData.get('id'), false);
//                 renderForm(formName);
//             }
...
window.updateForm = function (formName, tabNumber, formId = "") {
    ...
    let formSelector =
        formId && formId !== "" ? `#${formId}` : `#profile-form-${formName}`;
...
document.addEventListener("click", function (event) {
    if (event.target.parentElement.classList.contains("delete-btn")) {
        const formName =
            event.target.parentElement.getAttribute("data-form-name");
...
window.deleteForm = function (formName, id) {
    // Translation object for delete confirmation and messages
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: `/api/candidate-portal/profiles/delete/${formName}/${id}`,
                type: "GET",
                success: function (response) {
                    Swal.fire({
                        icon: "success",
...
$(document).ready(function () {
    ...
    $("#languages-language").on("change", function () {
        ...
        if (languageId) {
            ...
            $.ajax({
                url: "/api/candidate-portal/profiles/get-language-skills", // Backend route to get language skills
                type: "GET",
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        $("#language-skills-container").html(
                            "<p>Aucune compétence disponible pour cette langue.</p>"
                        );
                    }
                },
                error: function () {
                    $("#language-skills-container").html(
                        "<p>Erreur lors du chargement des compétences linguistiques.</p>"
                    );
                },
            });
        } else {
...
window.toggleFormationForm = function (formName, formationId, showForm) {
    const viewElement = document.getElementById(
        `view-${formName}-${formationId}`
    );
    const formElement = document.getElementById(
        `form-${formName}-${formationId}`
    );
...
window.deleteCard = function (formName, id) {
    ...
    if (formName === "formations") {
        ...
        recommandations = recommandations.filter(
            (recommandation) => recommandation.id !== id
        );
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSelectedDisplay() {
        ...
        selectedDisplay.textContent =
            selected.length > 0 ? selected.join(", ") : "Permis"; // Fallback text
    }
