...
async function updateDescription() {
    ...
    try {
        ...
        const destinatairName = {
            first_name: destinatairData.first_name || '[Prénom]',
            last_name: destinatairData.last_name || '[Nom]'
        };

        let subject = '';
        let body = '';

        // Map form event types to database template types
        let apiEventType = '';
        if (eventType === 'presentiel') {
            apiEventType = 'physique';
        } else if (eventType === 'visioconference') {
            apiEventType = 'distanciel';
        } else if (eventType === 'telephonique') {
            apiEventType = 'telephonique';
        }

        if (apiEventType) {
            const emailContent = await generateEmailContent(apiEventType, {
                destinatairName,
                professionName,
                eventDate,
                startTime,
        ...
        if (descriptionEditor && body) {
            // Convert newlines to HTML for CKEditor if needed
            const htmlBody = body.includes('<') ? body : body.replace(/\n/g, '<br>');
            descriptionEditor.setData(htmlBody);
        }
    } catch (error) {
...
async function generateEmailContent(eventType, eventData) {
    console.log('Generating email content for:', eventType, eventData); // Debug log
    ...
    try {
        const payload = {
            ...
            event_url: eventData.eventUrl || null
        };
        ...
        if (result.success) {
            return {
                ...
                body: result.content || result.body // Handle both 'content' and 'body' keys
            };
        } else {
        ...
        // Return fallback content that matches your database templates
...
function generateFallbackContent(eventType, eventData) {
    const templates = {
        'physique': {
            subject: `Convocation à un entretien en présentiel – ${eventData.destinatairName.last_name} ${eventData.destinatairName.first_name} - ${eventData.professionName}`,
            body: `Bonjour ${eventData.destinatairName.first_name},\n\nNous vous invitons à un entretien en présentiel dans le cadre de votre recrutement pour le poste ${eventData.professionName}.\n\nDate : ${formatDate(eventData.eventDate)}\nHeure : ${formatTime(eventData.startTime)}\nLieu : ${eventData.location || '[Lieu de l\'entretien]'}\n\nMerci de bien vouloir confirmer votre présence en réponse à cet email.\n\nPour toute question ou difficulté technique, merci de nous écrire à support@humanjobs.ma.\n\nBien cordialement,\nL'équipe Humanjobs.`
        },
        'distanciel': {
            subject: `Convocation à un entretien en visioconférence – ${eventData.destinatairName.last_name} ${eventData.destinatairName.first_name} - ${eventData.professionName}`,
            body: `Bonjour ${eventData.destinatairName.first_name},\n\nNous vous invitons à un entretien à distance dans le cadre de votre recrutement pour le poste ${eventData.professionName}.\n\nDate : ${formatDate(eventData.eventDate)}\nHeure : ${formatTime(eventData.startTime)}\nLien : ${eventData.eventUrl || '[Lien de visioconférence]'}\n\nMerci de bien vouloir confirmer votre présence en réponse à cet email.\n\nPour toute question ou difficulté technique, merci de nous écrire à support@humanjobs.ma.\n\nBien cordialement,\nL'équipe Humanjobs.`
        },
        'telephonique': {
            subject: `Convocation à un entretien téléphonique – ${eventData.destinatairName.last_name} ${eventData.destinatairName.first_name} - ${eventData.professionName}`,
            body: `Bonjour ${eventData.destinatairName.first_name},\n\nNous vous invitons à un entretien téléphonique dans le cadre de votre recrutement pour le poste ${eventData.professionName}.\n\nDate : ${formatDate(eventData.eventDate)}\nHeure : ${formatTime(eventData.startTime)}\n\nMerci de bien vouloir confirmer votre présence en réponse à cet email.\n\nPour toute question ou difficulté technique, merci de nous écrire à support@humanjobs.ma.\n\nBien cordialement,\nL'équipe Humanjobs.`
        }
    };
...
function getDestinatairName(participantIds, model) {
    if (!participantIds || participantIds.length === 0) {
        return Promise.resolve({ first_name: '[Prénom]', last_name: '[Nom]' });
    }
    ...
    return $.ajax({
        ...
        return { first_name: '[Prénom]', last_name: '[Nom]' };
    });
}
...
window.saveEvent = function (type) {
    ...
    function createEditorAndLoadData(eventId) {
        const el = document.querySelector('#edit_description');
        if (!el) {
            console.error('Element #edit_description not found in DOM.');
            return;
        }
        ...
        ClassicEditor
            ...
            .then(editor => {
                ...
                // Charger les données après création de l'éditeur
    ...
    function loadEventData(eventId, editor) {
        $.ajax({
            ...
            success: function (response) {
                ...
                // Set radio button for event type
                const eventTypeRadio = $(`#edit_event_${response.event_type}`);
                if (eventTypeRadio.length) {
                    eventTypeRadio
                        .prop('checked', true)
                        .trigger('change')
                        .next('label')
                        .addClass('activated active');
                }
                ...
                const destinatairesIds = (response.destinataires || []).map(dest => dest.id);
                const participantsIds = (response.participants || []).map(part => part.id);

                const destinatairesMeta = (response.destinataires || []).reduce((acc, dest) => {
                    acc[dest.id] = dest.type;
                    return acc;
                }, {});

                const participantsMeta = (response.participants || []).reduce((acc, part) => {
    ...
    // Détruire l'éditeur s'il existe déjà
    const existingEditor = window.descriptionEditors['edit_description'];
    if (existingEditor) {
...
function viewEvent(eventId) {
    // Initialiser l'objet global CKEditor s'il n'existe pas
    if (typeof window.descriptionEditors === 'undefined') {
        window.descriptionEditors = {};
    }

    function createEditorAndLoadData(eventId) {
        const el = document.querySelector('#show_description');
        if (!el) {
            console.error('Element #show_description not found in DOM.');
            return;
        }
    ...
    function loadEventData(eventId, editor) {
        $.ajax({
            ...
            success: function (response) {
                ...
                // Radio event type
                const eventTypeRadio = $(`#show_event_${response.event_type}`);
                if (eventTypeRadio.length) {
                    eventTypeRadio
                        .prop('checked', true)
                        .trigger('change')
    ...
    // Détruire l'éditeur s'il existe
    const existingEditor = window.descriptionEditors['show_description'];
    if (existingEditor) {
        existingEditor.destroy()
            .then(() => {
                delete window.descriptionEditors['show_description'];
    ...
    function setupEventTypeToggle(modalSelector) {
        const modal = document.querySelector(modalSelector);
        if (!modal) return;

        const eventTypeRadios = modal.querySelectorAll("input[name='event_type']");
        const urlInputContainer = modal.querySelector(".visio-part");
        const locationInputContainer = modal.querySelector(".location-part");

        function toggleUrlInput() {
            const selectedType = modal.querySelector("input[name='event_type']:checked")?.value;
            console.log("Selected event type:", selectedType); // Debugging line
            if (selectedType === "visioconference") {
        ...
        // Run on modal show (in case values are pre-filled)
        modal.addEventListener('shown.bs.modal', toggleUrlInput);

        // Also run initially in case modal is already visible
        toggleUrlInput();
    }

    // Call for both modals
    setupEventTypeToggle('#editEventModal');
    setupEventTypeToggle('#createEventModal');
