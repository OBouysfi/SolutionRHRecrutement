...
@section('title',  __('generated.Modifier un client'))
    ...
    <main class="main mainheight">
        ...
        <div class="container-fluid">
            ...
            <nav aria-label="breadcrumb" class="breadcrumb-theme">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active " aria-current="page"><span >{{ __("generated.Modifier client :") }}</span>  {{ __($client->name) }}</li>
...
{{--                            <a style="font-size: 14px" id="filiales-tab" onclick="openTab('filiales')"--}}
{{--                                class="nav-link ">Filiales</a>--}}
{{--                        </li>--}}
            ...
            <div class="row">
                <div class="tab-content py-3" id="myTabContent">
                    @include('client.inc.generalInformation', ['client' => $client])
...
@section('js_footer')
    ...
    <script>
       ...
       var countries = @json($countries);
        ...
        var ApiClientEditData = "{{ route('api.client.edit', $client->id) }}";
        var ApiClientFinancesEditData = @json($clientFinance) ?
            "{{ route('api.client.finance.edit', $clientFinance?->id) }}" : null;
        var ApiClientFinancesCreateData = !@json($clientFinance) ? "{{ route('api.client.finance.create') }}" :
            null;
        var ApiClientFilialesEditData = "{{ route('api.client.filiale.edit') }}";
        var ApiClientSitesEditData = "{{ route('api.client.site.edit') }}";
        var ApiClientEditEvaluators = "{{ route('api.evaluator.edit') }}";
        var ApiClientDeleteEvaluator = "{{ route('api.evaluator.delete', ['evaluatorId' => 'evaluatorId']) }}";
        var ApiClientDeleteFiliale = "{{ route('api.client.filiale.delete', ['filialeId' => 'filialeId']) }}";
        var ApiClientDeleteSite = "{{ route('api.client.site.delete', ['siteId' => 'siteId']) }}";
        var ApiClientEditAdminUser = "{{ route('api.client.account.user.store') }}";
            ...
            function addImagesToChosen() {
                $('.chosen-results li').each(function() {
                    ...
                    if (code) {
                        ...
                        var imgTag = `<img src="https://flagcdn.com/w160/${code}.png" srcset="https://flagcdn.com/w320/${code}.png 2x" width="40"
