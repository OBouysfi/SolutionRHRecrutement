<div class="modal fade" id="ModalConversionShortlistForEmbauches" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content bg-gradient-theme-light">
            <div class="modal-header">
                <div class="row align-items-center" style="width: 100%">
                    ...
                    <div class="col">
                        <h6 class="fw-medium mb-0 ">{{ __("generated.Taux de conversion shortlist vers embauches") }}</h6>
                    </div>
                            ...
                            <div
                                ...
                                <div class="rounded date-annee" style="float: left;width: 76%;margin-left: 10px;">
                                    <select class="form-select form-select-sm border-0 bg-transparent w-100"
                                        id="btn-detail-conversion_shortlist_embauches-year"
                                        name="btn-detail-conversion_shortlist_embauches-year" style="float: left">
                                        @php
                                            $currentYear = date('Y');
                                        @endphp
            ...
            <div class="modal-body d-block">
                <div class="row  mb-3">
                    <div class="col-12">
                        <table class="table" id="conversion_shortlist_embauches" style="width: 100%">
                            <thead style="font-size: 14px">
                                <tr>
                                    <th  style="width: 123px;">{{ __("generated.Mois") }}</th>
                                    <th  style="text-align: right;">{{ __("generated.Shortlist") }}</th>
                                    <th  style="text-align: right;">{{ __("generated.Embauches") }}</th>
                                    <th  style="text-align: right;">{{ __("generated.Taux de conversion") }}</th>
                                </tr>
                            </thead>
                            <tbody style="font-size: 14px">
                                @foreach ($monthlyShortlistForHiring_conversionRates as $index => $rate)
                                    <tr>
                                        <td class=" translated_text">{{ __($monthlyShortlistForHiring_labels[$index]) }}</td>
                                        <td class=" translated_text" style="text-align: right;">
                                            {{ __($monthlyShortlistForHiring_shortlisted[$index]) }}</td>
                                        <td class=" translated_text" style="text-align: right;">{{ __($monthlyShortlistForHiring_hired[$index]) }}
                                        </td>
                                        <td class=" translated_text" style="text-align: right;">{{ __($rate) }}%</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    function loadChart_conversion_shortlist_embauches(chartType, selectedYear) {
        $.ajax({
            ...
            success: function(response) {
                let tableBody = $('#conversion_shortlist_embauches tbody');
                tableBody.empty();

                let data = response.conversion_shortlist_embauches;
                ...
                if (Array.isArray(data.monthlyLabels) && data.monthlyLabels.length > 0) {
                    data.monthlyLabels.forEach(function(month, index) {
                        ...
                        <tr>
                            ...
                            <td class=" translated_text" style="text-align: right;">${data.monthlyShortlisted[index]}</td>
                            <td class=" translated_text" style="text-align: right;">${data.monthlyHired[index]}</td>
    ...
    $('#btn-detail-conversion_shortlist_embauches-year').on('change', function() {
        let selectedYear = $('#btn-detail-conversion_shortlist_embauches-year').val();
        loadChart_conversion_shortlist_embauches('conversion_shortlist_embauches', selectedYear);
    });
</script>
