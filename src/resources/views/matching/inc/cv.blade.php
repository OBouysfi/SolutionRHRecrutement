...
@section('title', __('generated.Consulter CV'))
...
<main class="main mainheight">
    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb" class="breadcrumb-theme">
            <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">{{ __("generated.Curriculum Vitae") }} : {{ __($profile->first_name) }} {{ __($profile->last_name) }}</li>
            </ol>
        </nav>
    </div>
                        ...
                        <div class="card border-0" style="width: 657px;margin-right: 44px;">
                            <div class="card-body">
                                ...
                                <div class="col-12 text-center">
                                    ...
                                    @php
                                    $cvUrl = asset('storage/' . $profile->path_cv_manual);
                                    $extension = strtolower(pathinfo($profile->path_cv_manual, PATHINFO_EXTENSION));
                                    @endphp

                                    @if($extension === 'pdf')
                                    <iframe src="{{ __($cvUrl) }}" width="100%" height="800px" style="border: none;"></iframe>

                                    @elseif(in_array($extension, ['doc', 'docx']))
                                    <a href="{{ __($cvUrl) }}" class="btn btn-primary" target="_blank">
                                        Télécharger la lettre de motivation (Word)
                                    </a>
                                    @else
                                    <p>{{ __("generated.Format de CV non supporté.") }}</p>
                                    @endif
                                    @else
                                    <p>{{ __("generated.Aucune CV disponible.") }}</p>
                                    @endif
