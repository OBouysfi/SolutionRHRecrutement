    ...
    <main class="main mainheight">
        ...
        <div class="container-fluid">
            <div class="row align-items-center page-title">
                <div class="col-12 col-md mb-2 mb-sm-0">
                    <h5 class="mb-0">{{ __('generated.cvtheque') }}</h5>
                </div>
                ...
                <div class="col col-sm-auto" style="margin-right: -14px;" data-bs-toggle="tooltip" data-bs-placement="top"
                    title="{{ __('generated.guide_video') }}">
            ...
            <nav aria-label="breadcrumb" class="breadcrumb-theme">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active" aria-current="page">{{ __('generated.Modifier un CV') }}</li>
        ...
        <div class="container mt-4">
                ...
                <div class="col-5">
                    <ul class="nav nav-tabs nav-adminux footer-tabs nav-fill" id="navtabscard30" role="tablist"
                        ...
                        <li class="nav-item d-flex justify-content-center" role="presentation">
                            <a class="nav-link active text-center translated_text" id="automatique-tab" data-bs-toggle="tab"
                                href="#automatique" role="tab" aria-controls="automatique"
                                aria-selected="true">Automatique</a>
                        </li>
                        <li class="nav-item d-flex justify-content-center translated_text" role="presentation">
                            <a class="nav-link text-center" id="manuel-tab" data-bs-toggle="tab" href="#manuel"
                                role="tab" aria-controls="manuel" aria-selected="false">Manuel</a>
                        </li>
                    </ul>
                </div>
            </div> --}}
            <div class="row">
                ...
                {{-- @include('candidate_portal.profile.inc.editAuto') --}}
                @include('candidate_portal.profile.inc.editManual')
                {{-- </div> --}}
            </div>
        </div>
    </main>
...
<script>
    ...
    <script>
        ...
        function activateTab(tabId) {
            // Remove 'active' and 'show' from all tabs and content
            document.querySelectorAll('#myTab .nav-link').forEach(tab => tab.classList.remove('active'));
            document.getElementById('sub-personal1').classList.remove('active', 'show');

            // Find the tab and its content
            let tabButton = document.getElementById("sub-personal" + tabId + "-tab");
            let tabContent = document.getElementById("sub-personal" + tabId);
        ...
        // Load the stored tab when the page opens
        window.addEventListener('DOMContentLoaded', function() {
            let savedTab = localStorage.getItem('activeTab') || '1';
            console.log("Loading saved tab:", "sub-personal" + savedTab + "-tab");

            if (document.getElementById("sub-personal" + savedTab + "-tab")) {
                activateTab(savedTab); // Only activate if the tab exists
            } else {
                console.warn("Tab not found:", "sub-personal" + savedTab + "-tab");
            }
        });
    </script>
    <script>
    ...
    <script>
        ...
        const contractsTypes = @json($contractsTypes);
